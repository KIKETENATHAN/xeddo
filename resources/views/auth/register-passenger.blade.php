...
<head>
    ...
    <title>Register as Passenger - Xeddo</title>
        ...
        <div class="max-w-md w-full space-y-8">
            <div>
                ...
                <p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                    Join Xeddo and start booking rides
                </p>
            </div>
            
            <form class="mt-8 space-y-6" action="{{ route('register.passenger.store') }}" method="POST">
                @csrf
                <div class="rounded-md shadow-sm -space-y-px">
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" required 
                                  class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 placeholder-gray-500 dark:placeholder-gray-400 text-gray-900 dark:text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm dark:bg-gray-700" 
                                  placeholder="Address" rows="3">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="text-center">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Want to be a driver? 
                    <a href="{{ route('register.driver') }}" class="font-medium text-green-600 hover:text-green-500">
                        Register as Driver
                    </a>
                </p>
            </div>
